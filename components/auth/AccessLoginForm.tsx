...
const CLIENT_HERO = {
  title: "Seguí el avance de tu inversión con total transparencia.",
  highlight: "transparencia",
  subtitle:
    "Consultá el progreso de tus unidades, recibí novedades y accedé a toda la información del proyecto desde un solo lugar.",
}
...
export function AccessLoginForm({ audience }: AccessLoginFormProps) {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      const access = await finalizeAccessLogin(audience)
      if (!access.ok) {
        await signOutClient()
      ...
      router.replace(
        audience === "equipo" && safeNextPath ? safeNextPath : access.redirectTo,
      )
    } catch {
  ...
  return (
    <AuthSplitLayout hero={audience === "cliente" ? CLIENT_HERO : undefined}>
      <AuthFormCard>
