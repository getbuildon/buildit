/** Tokens Figma — Dashboard (node 1157:2806) */
export const DASHBOARD_SHADOW = "0 0 5px rgba(243, 103, 31, 0.08)"

export const DASHBOARD_TYPE = {
  pageTitle: "font-recoleta text-[36px] font-normal leading-[1.05] text-[#272a2d]",
  pageSubtitle: "text-[14px] font-normal leading-[1.4] text-[#43484e]",
  statValue: "font-recoleta text-[28px] font-normal leading-[1.05] text-[#111113] tabular-nums",
  statLabel: "text-[12px] font-normal leading-[1.4] tracking-[-0.36px] text-[#212225]",
  statSublabel: "text-[12px] font-normal leading-[1.4] tracking-[-0.36px] text-[#696e77]",
  floorName: "w-[100px] shrink-0 text-[14px] font-semibold leading-[1.4] tracking-[-0.15px] text-[#43484e]",
  floorProgress: "shrink-0 text-[12px] font-medium leading-[1.4] text-[#62748e]",
  floorMeta: "text-[12px] font-normal leading-[1.4] tracking-[-0.36px] text-[#5a6169]",
  unitCode: "text-[10px] font-normal leading-[1.4] tracking-[-0.5px] text-[#43484e]",
  unitProgress: "text-[10px] font-normal leading-[1.4] tracking-[-0.5px] text-[#777b84]",
  unitType: "truncate text-[10px] font-normal leading-[1.4] tracking-[-0.5px] text-[#272a2d]",
} as const
